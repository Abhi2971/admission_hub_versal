...
import { getCollegeApplications, approveApplication, rejectApplication } from '../../services/admin';
...
const ReviewApplications = () => {
  ...
  const fetchApplications = useCallback(async () => {
    try {
      ...
      const response = await getCollegeApplications(params);
  ...
  }, [hasActive, fetchApplications]);

  const handleApprove = async (appId, action) => {
    try {
      await approveApplication(appId, action);
  ...
  if (!hasActive) return <SubscriptionRequired feature="review applications" />;
            ...
            return (
              <div key={app._id} style={styles.appCard}>
                ...
                <div style={styles.appActions}>
                  <Link to={`/admin/applications/${app._id}/documents`} style={styles.docLink}>
                    View Documents
                  </Link>
                  ...
                  {app.status === 'applied' && (
                    <>
                      <button
                        onClick={() => handleApprove(app._id, 'shortlist')}
                  ...
                  {app.status === 'shortlisted' && (
                    <>
                      <button
                        onClick={() => handleApprove(app._id, 'offer')}
