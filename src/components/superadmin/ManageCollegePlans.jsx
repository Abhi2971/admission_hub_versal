...
import { getCollegePlans, createCollegePlan, updateCollegePlan, deleteCollegePlan } from '../../services/superadmin';
import Alert from '../common/Alert';
import Loader from '../common/Loader';

const ManageCollegePlans = () => {
  ...
  const fetchPlans = async () => {
    try {
      const response = await getCollegePlans();
  ...
  const handleSubmit = async (e) => {
    ...
    const dataToSend = {
      ...
      max_courses: parseInt(formData.max_courses),
      max_students: parseInt(formData.max_students),
      features: formData.features.split(',').map(f => f.trim())
    };
    try {
      if (editingPlan) {
        await updateCollegePlan(editingPlan._id, dataToSend);
        setSuccess('Plan updated successfully');
      } else {
        await createCollegePlan(dataToSend);
  ...
  const handleDelete = async (planId) => {
    ...
    try {
      await deleteCollegePlan(planId);
  ...
  return (
    <div style={styles.container}>
      <h1 style={styles.title}>Manage College Plans</h1>
        ...
        <div style={styles.formCard}>
          ...
          <form onSubmit={handleSubmit}>
            <div style={styles.formGrid}>
              ...
              <div style={styles.formGroup}>
                <label style={styles.label}>Max Courses</label>
                <input
                  type="number"
                  name="max_courses"
              ...
              <div style={{ ...styles.formGroup, ...styles.formGroupFull }}>
                ...
                <input
                  ...
                  onChange={handleChange}
                  placeholder="e.g., Course management, Application review, Analytics"
                  required
                  style={styles.input}
                />
              </div>
        ...
        <table style={styles.table}>
          <thead style={styles.thead}>
            <tr>
              ...
              <th style={styles.th}>Max Courses</th>
          ...
          <tbody>
            {plans.map(plan => (
              <tr key={plan._id} style={styles.tr}>
                ...
                <td style={styles.td}>{plan.billing_period}</td>
                <td style={styles.td}>{plan.max_courses}</td>
                <td style={styles.td}>{plan.max_students}</td>
                <td style={styles.td}>
                  <span style={{
                    ...styles.statusBadge,
...
export default ManageCollegePlans;
